...
class WeatherServiceDarksky {
    ...
    class func formatPrecipString(uStrings: UnitsHelper.UnitsStrings, condition:DarkSkyTypes.Condition, value: Double) -> String {
        ...
        if uStrings.sys == .us {
            valueMM = value * 25.2
        }else if unit == uStrings.precipSnowStr && uStrings.sys == .ca {
            valueMM = value * 10
        }else{
            valueMM = value
        }
        ...
        if isSnow {
            switch valueMM {
            case 0..<2 : r="light flurries"
            case 2..<10 : r="snow "+String(format: "%.1f", value)+" \(unit)"
            case 10... : r="snow "+String(format: "%.0f", value)+" \(unit)"
            default: r="snow "+String(format: "%.0f", value)+" \(unit)"
            }
            }else{
            switch valueMM {
            case 0..<1 : r="light rain"
            case 1..<3 : r="rain "+String(format: "%.1f", value)+" \(unit)"
            case 3... : r="rain "+String(format: "%.0f", value)+" \(unit)"
